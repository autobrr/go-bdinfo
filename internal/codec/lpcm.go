...
func ScanLPCM(a *stream.AudioStream, data []byte) {
	...
	// Divergence from BDInfo: TSCodecLPCM initializes on a short header, and the guard
	// above then skips every later scan, so the zeroed fields stick. See AGENTS.md.
	if len(data) < 4 {
		return
	}
