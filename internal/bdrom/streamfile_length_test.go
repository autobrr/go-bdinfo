...
func TestStreamFileLengthUsesDTSOnly(t *testing.T) {
	...
	s.handleTimestamp(nil, states, 0x1011, state, 1000, 1000, true, &first, &last)
	if s.Length != 0 {
	...
	s.handleTimestamp(nil, states, 0x1011, state, 2000, 2000, true, &first, &last)
	if s.Length != 0 {
	...
	s.handleTimestamp(nil, states, 0x1011, state, 3000, 2000, true, &first, &last)
	if s.Length != 0 {
		t.Fatalf("expected length 0 after PTS-only timestamp, got %v", s.Length)
	}

	// Next DTS-bearing timestamp extends the file duration.
	s.handleTimestamp(nil, states, 0x1011, state, 5000, 5000, true, &first, &last)
