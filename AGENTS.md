...
- Initialization after a failed parse: `TSCodecLPCM.Scan` sets `IsInitialized` even when the payload is under 4 bytes, and `TSCodecPGS.ReadPCS` does the same when the segment is truncated before the video size. The `if IsInitialized` guard then locks the zeroed fields in for the rest of the disc. Go (`internal/codec/lpcm.go`, `internal/codec/pgs.go`) leaves the stream uninitialized on a short payload, so a later transfer or stream file can fill it. Only reachable on corrupt or crafted streams; real discs are unaffected.
